...
impl InputDeviceKeyManager for InputDeviceKeyManagerImpl {
    ...
    fn create_input_device_key(&self, device: &Device, input_device_entity_instance: Arc<ReactiveEntityInstance>, key: Key) {
        ...
        match input_device_key {
            Ok(input_device_key) => {
                ...
                self.create_key_event(input_device_entity_instance.clone(), input_device_key.clone());
            }
            Err(_) => {
                error!("Failed to create entity instance for input device key {}!", unique_name);
            }
        }
    }
